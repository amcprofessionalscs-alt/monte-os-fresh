      ...
      <div style={{ maxWidth: '390px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 2, paddingBottom: '190px' }}>
        <div style={{ padding: '52px 0 0' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '22px' }}>
            ...
            <button onClick={handleSignOut} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', color: 'rgba(255,255,255,0.22)', fontSize: '10px', fontFamily: FONT_MONO, padding: '6px 14px', cursor: 'pointer', letterSpacing: '0.08em', minHeight: '36px' }}>sign out</button>
          </div>
        ...
        {habits.map((habit, i) => (
          <motion.div key={habit.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 + i * 0.14, duration: 0.4, ease: [0.22, 1, 0.36, 1] }}>
            <HabitCard habit={{ id: String(habit.id), name: habit.name, category: CAT_MAP[habit.name] ?? 'monte', reps: habit.current_reps, target: habit.target_reps }} onAddRep={handleAddRep} onRemoveRep={handleRemoveRep} />
          </motion.div>
        ))}
      </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 20, padding: '20px 20px calc(16px + env(safe-area-inset-bottom,0px))', background: 'linear-gradient(to top,rgba(10,13,26,1) 0%,rgba(10,13,26,0.97) 50%,transparent 100%)' }}>
        <div style={{ maxWidth: '390px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'stretch', marginBottom: '6px' }}>
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} onClick={() => setChatOpen(true)} style={{ flex: 1, borderRadius: '16px', background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.28)', color: '#fbbf24', fontFamily: FONT_SYNE, fontWeight: 800, fontSize: '12px', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', minHeight: '52px', cursor: 'pointer', animation: 'chat-fab-glow 3s ease-in-out infinite' }}>⚡ Brain Chat</motion.button>
            <IgnitionButton />
          </div>
          <Link href="/history" style={{ display: 'block', padding: '10px', color: 'rgba(255,255,255,0.15)', fontFamily: FONT_MONO, fontSize: '10px', letterSpacing: '0.15em', textTransform: 'uppercase', textDecoration: 'none', textAlign: 'center' }}>VIEW IGNITION LOG</Link>
        </div>
      </div>
